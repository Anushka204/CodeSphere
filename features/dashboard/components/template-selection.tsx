...
const TemplateSelectionModal= ({isOpen,onClose,onSubmit}:TemplateSelectionModalProps) => {
  ...
  const [category,setCategory]=useState<"all"|"frontend"|"backend"|"full stack">("all")
      ...
      <DialogContent className='sm:max-w-[800px] max-h-[90vh] overflow-y-auto'>
{
  ...
  <>
  <DialogHeader>
          <DialogTitle className='text-2xl font-bold text-[#8f37ad] flex items-center gap-2'>
            <Plus size={24} className='text-[#8f37ad]'></Plus>
            Select a Template
          </DialogTitle>
          <DialogDescription>
...
filteredTemplates.map((template)=>(
   <div
                        ...
                        className={`relative flex p-6 border rounded-lg cursor-pointer
                          ...
                          ${
                            selectedTemplate === template.id
                              ? "border-[#973fe9]  shadow-[0_0_0_1px_#E93F3F,0_8px_20px_rgba(233,63,63,0.15)]"
                              : "hover:border-[#d682e1] shadow-[0_2px_8px_rgba(0,0,0,0.05)] hover:shadow-[0_8px_20px_rgba(0,0,0,0.1)]"
                          }`}
                      ...
                      >
                        ...
                        {selectedTemplate === template.id && (
                          <div className="absolute top-2 left-2 bg-[#8124c9] text-white rounded-full p-1">
                            <Check size={14} />
                          </div>
                        )}
                ...
                <div className='col-span-2 flex flex-col items-center justify-center p-8 text-center'>
                  <Search size={48} className='text-gray-300 mb-4'></Search>
          ...
          <div className=' flex justify-between gap-3 mt-4 pt-4 border-t'>
            <div className='flex items-center text-sm text-muted-foreground'>
              <Clock size={14} className='mr-1'></Clock>
              <span>Estimated setup time:{""}
                {selectedTemplate? "2-5 minutes":"Select a template"}
              </span>
            </div>
            <div className='flex gap-3'>
              <Button variant={"outline"} onClick={onClose}>Cancel</Button>
              <Button className='bg-[#7040c4] hover:bg-[#a977cb] text-white' onClick={handleContinue}>Continue <ChevronRight size={16} className="ml-1"></ChevronRight></Button>
            </div>
          </div>
        </div>
        </>
    ...
    <DialogHeader>
      <DialogTitle className='text-2xl font-bold text-[#9e35f4]'>Configure your Project</DialogTitle>
      <DialogDescription>
